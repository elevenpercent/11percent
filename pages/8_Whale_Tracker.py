...
if not scan_btn:
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-top:1.5rem;">
        ...
        <div class="panel-sm">
            ...
            <div style="font-size:0.82rem;color:#8896ab;line-height:1.65;">Daily OHLCV from Yahoo Finance. This shows historical volume anomalies, not real-time intraday order flow.</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
st.markdown(
    ...
    '<div class="stat-cell"><div class="stat-val ' + ("pos" if len(alerts) > 0 else "neu") + '">' + str(len(alerts)) + '</div><div class="stat-lbl">Alerts Found</div></div>'
    '<div class="stat-cell"><div class="stat-val neu">' + str(len(tickers)) + '</div><div class="stat-lbl">Symbols Scanned</div></div>'
    '<div class="stat-cell"><div class="stat-val neu">' + f"{std_thresh:.1f}σ" + '</div><div class="stat-lbl">Threshold</div></div>'
...
with feed_col:
    if not alerts:
        st.markdown(
            ...
            '</div>'
            '<div style="text-align:center;padding:3rem 1rem;color:#3a4a5e;font-size:0.8rem;font-family:IBM Plex Mono,monospace;">'
            f'No anomalies detected above {std_thresh:.1f}σ in the last {lookback}.<br>'
            'Try lowering the threshold or extending the lookback period.'
            '</div></div>',
            unsafe_allow_html=True
        )
    else:
        # Build rows as individual st.markdown calls to avoid f-string/HTML escaping issues
        # Header
        st.markdown(
            '<div style="background:#030508;border:1px solid #1a2235;border-radius:10px;overflow:hidden;">'
            '<div style="background:#08100d;border-bottom:1px solid #1a2235;padding:0.5rem 1rem;'
            'display:flex;align-items:center;gap:0.5rem;font-family:IBM Plex Mono,monospace;'
            'font-size:0.6rem;color:#3a4a5e;">'
            '<div style="width:8px;height:8px;border-radius:50%;background:#ff3d57;flex-shrink:0;"></div>'
            '<div style="width:8px;height:8px;border-radius:50%;background:#ffd166;flex-shrink:0;"></div>'
            '<div style="width:8px;height:8px;border-radius:50%;background:#00e676;flex-shrink:0;"></div>'
            f'<span style="margin-left:0.5rem;">WHALE ALERT FEED — {len(alerts)} events detected</span>'
            '<span style="margin-left:auto;font-size:0.52rem;">DATE · TICKER · VOL SPIKE · CHANGE · PRICE · Z</span>'
            '</div>'
        ...
        for a in alerts[:40]:
            ...
            row_html = (
                '<div style="border-bottom:1px solid #0d1117;padding:0.55rem 0.2rem;'
                'display:flex;align-items:center;gap:1rem;flex-wrap:wrap;'
                'font-family:IBM Plex Mono,monospace;">'
                f'<span style="color:#3a4558;font-size:0.6rem;flex-shrink:0;width:80px;">{date_str}</span>'
                f'<span style="color:#eef2f7;font-weight:700;font-size:0.82rem;flex-shrink:0;width:90px;">{a["ticker"]}{alert_tag}</span>'
                f'<span style="flex:1;min-width:120px;font-size:0.75rem;">'
                f'<span style="color:{z_color};font-weight:700;">{a["mult"]:.1f}×</span>'
                f'<span style="color:#3a4558;"> avg vol &nbsp;</span>'
                f'<span style="color:#8896ab;">{vol_fmt} vs {avg_fmt}</span>'
                f'</span>'
                f'<span style="color:{pct_color};font-size:0.8rem;flex-shrink:0;min-width:64px;text-align:right;">{pct_arrow} {abs(a["pct"]):.2f}%</span>'
                f'<span style="color:#8896ab;flex-shrink:0;font-size:0.72rem;min-width:64px;text-align:right;">${a["price"]:,.2f}</span>'
                f'<span style="color:{z_color};flex-shrink:0;font-size:0.65rem;min-width:36px;text-align:right;">{a["z"]:.1f}σ</span>'
                '</div>'
            )
...
with info_col:
    ...
    for t, s in summaries.items():
        ...
        st.markdown(
            f'<div style="background:#0c1018;border:1px solid {border_col};border-radius:8px;padding:0.8rem;margin-bottom:0.5rem;">'
            f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.3rem;">'
            f'<span style="font-size:0.82rem;color:#eef2f7;font-weight:700;">{t}</span>'
            f'<span style="color:{pct_color};font-size:0.72rem;">{s["last_pct"]:+.2f}%</span>'
            f'</div>'
            f'<div style="display:flex;justify-content:space-between;font-size:0.65rem;color:#3a4558;">'
            f'<span>Vol {vol_mult:.1f}× avg</span>'
            f'<span style="color:{"#b388ff" if has_alert else "#3a4558"};">{alert_count} alert{"s" if alert_count!=1 else ""}</span>'
            f'</div></div>',
            unsafe_allow_html=True
        )
