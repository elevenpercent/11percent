...
if not run_btn:
    ...
    for col, title, items in [
        (g1, "[G] Start here",       [("SMA Crossover","Visual, easy to understand"),("EMA Crossover","Slightly faster signals"),("RSI","Clear buy/sell levels")]),
        (g2, "[chart] Trending markets", [("SuperTrend","Clean ATR-based signals"),("MACD + SuperTrend","Dual confirmation"),("EMA + RSI Filter","Trend + momentum")]),
...
wr = float(m["win_rate"])
verdict = "OK Beat buy & hold" if alpha > 0 else "(!) Underperformed buy & hold"
...
wr_word = "above" if wr >= 50 else "below"
trade_note = "Overtrading risk" if m["num_trades"] > 60 else ("Low sample size" if m["num_trades"] < 8 else "Trade count OK")
st.markdown(f"""
<div style="background:#0d1117;border:1px solid #1a2235;border-radius:8px;padding:1rem 1.4rem;margin:0.8rem 0;display:flex;gap:2rem;flex-wrap:wrap;font-family:'IBM Plex Mono',monospace;font-size:0.72rem;">
    ...
    <span style="color:#3a4558;">*</span>
    <span style="color:#8892a4;">Win rate: {wr_word} 50%</span>
    <span style="color:#3a4558;">*</span>
    <span style="color:#8892a4;">{trade_note}</span>
</div>
""", unsafe_allow_html=True)
