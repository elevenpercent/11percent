...
with t2:
    ...
    <div style="background:#0c1018;border:1px solid #1a2235;border-radius:6px;padding:0.75rem;margin:0.5rem 0;font-family:'IBM Plex Mono',monospace;font-size:0.72rem;">
        ...
        <div style="display:flex;justify-content:space-between;color:#3a4a5e;">
            <span>Realised P&L</span><span style="color:{'#00e676' if cp>=avg_cost else '#ff3d57'};">${(cp-avg_cost)*sq:+,.2f}</span>
        </div>
    </div>""", unsafe_allow_html=True)
...
with t3:
    ...
    st.markdown(f"""
    <div style="background:#0c1018;border:1px solid #1a2235;border-radius:{10}px;padding:1.2rem;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:1rem;">
            ...
            <div class="metric-card" style="padding:0.7rem;"><div class="metric-val neu" style="font-size:1rem;">${avg_cost:.2f if avg_cost else "—"}</div><div class="metric-lbl">Avg Cost</div></div>
        ...
        <div style="border-top:1px solid #1a2235;padding-top:0.8rem;font-family:'IBM Plex Mono',monospace;font-size:0.7rem;color:#3a4a5e;">
            vs Buy &amp; Hold: <span style="color:{'#00e676' if pnl>=bh_pnl else '#ff3d57'};">{('Beating' if pnl>=bh_pnl else 'Trailing')} B&H by ${abs(pnl-bh_pnl):,.2f}</span>
        </div>
    </div>""", unsafe_allow_html=True)
