...
.step-num { font-family:'Bebas Neue',sans-serif; font-size:4rem; color:#1a2235; line-height:1; margin-bottom:0.3rem; }
...
.sc-card-hdr { padding:0.9rem 1.2rem; border-bottom:1px solid #1a2235; background:#08100d;
    font-family:'IBM Plex Mono',monospace; font-size:0.55rem; text-transform:uppercase; letter-spacing:0.2em; color:#3a4a5e; }
.strat-row { display:flex; align-items:flex-start; gap:0.8rem; padding:0.75rem 1.2rem; border-bottom:1px solid #0d1117; transition:background 0.15s; }
.strat-row:last-child{border-bottom:none} .strat-row:hover{background:rgba(255,255,255,0.015)}
.strat-tag { display:inline-block; font-family:'IBM Plex Mono',monospace; font-size:0.5rem; text-transform:uppercase;
    letter-spacing:0.12em; padding:2px 7px; border-radius:3px; white-space:nowrap; flex-shrink:0; margin-top:2px; }
...
.concept-row { display:grid; grid-template-columns:160px 1fr; gap:0.8rem; align-items:baseline;
...
if tape:
    items = "".join(
        ...
        f'<span class="{cls}">{p} {a} {ch}</span></span>'
        for s,p,ch,a,cls in tape
    )
    st.markdown('<div class="ticker-outer"><div class="ticker-track">'+items+items+'</div></div>', unsafe_allow_html=True)
...
with hero_l:
    bars = "".join(
        f'<div style="width:10px;height:{max(8,h*0.7)}px;background:{c};border-radius:1.5px;'
        f'animation:fadeUp 0.4s {i*0.04:.2f}s ease both;opacity:0.7;"></div>'
...
with hero_r:
    ...
    # Header + stats + market status (pure HTML — no JS needed here)
    st.markdown(
        ...
        'display:flex;align-items:center;gap:0.4rem;font-family:IBM Plex Mono,monospace;font-size:0.6rem;color:#3a4a5e;">'
        ...
        '</div>'
        '<div id="tv-hero-mount" style="height:280px;width:100%;"></div>'
        '</div>'
    ...
    # TV widget needs components.html to execute JS
    st.components.v1.html("""
<style>
body,html{margin:0;padding:0;background:#0c1018;}
</style>
<div style="height:280px;width:100%;background:#0c1018;">
...
with sc_l:
    rows = "".join(
        ...
        f'<div><div class="strat-name">{name}</div><div class="strat-desc">{desc}</div></div></div>'
