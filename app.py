...
.step-num {
    ...
    color:#1a2235;line-height:1;margin-bottom:0.3rem;
...
if market_name == “Crypto”: return “24/7”, “#00e676”, True
if time_ny.weekday() >= 5:  return “CLOSED (WKND)”, “#ff3d57”, False
s = time_ny.replace(hour=9, minute=30, second=0, microsecond=0)
e = time_ny.replace(hour=16, minute=0,  second=0, microsecond=0)
if s <= time_ny <= e: return “OPEN”, “#00e676”, True
...
st.markdown(’</div><div class="nb-tag"><span class="live-dot"></span>LIVE DATA</div></div>’, unsafe_allow_html=True)
...
if tape:
html = “”.join(f’<span class="ticker-item"><span class="t-sym">{s}</span><span class="{cls}">{p} {a} {ch}</span></span>’ for s,p,ch,a,cls in tape) * 2
st.markdown(f’<div class="ticker-wrap tape-wrap"><div class="ticker-tape">{html}</div></div>’, unsafe_allow_html=True)
...
with hero_l:
# Animated candles visual using HTML/CSS
...
ht = max(8, h * 0.7)
delay = i * 0.04
candles_html += f’<div style="width:10px;height:{ht}px;background:{col};border-radius:1.5px;animation:fadeUp 0.4s {delay:.2f}s ease both;opacity:0.7;"></div>’
candles_html += ‘</div>’
...
with hero_r:
# Mock TradingView-style terminal screen card
nyse_status, nyse_color, nyse_open = get_market_status(“NYSE”)
nas_status,  nas_color,  nas_open  = get_market_status(“NASDAQ”)
cme_status,  cme_color,  cme_open  = get_market_status(“CME Futures”)
cry_status,  cry_color,  cry_open  = get_market_status(“Crypto”)

```
# Build mini sparkline SVG for visual
...
w, h = 280, 60
sp_coords = " ".join(f"{(i/(len(sp_pts)-1))*w:.1f},{h - ((p-sp_min)/sp_range*h*0.85 + h*0.07):.1f}" for i,p in enumerate(sp_pts))
sparkline = f"""
<svg width="{w}" height="{h}" style="overflow:visible;">
    ...
    <polygon points="{sp_coords} {w},{h} 0,{h}" fill="url(#spGrad)"/>
</svg>"""
...
<div style="padding:2.5rem 0 1rem 0;" class="stat-grid">
    ...
    <div class="screen-card" style="margin-bottom:1rem;animation:fadeUp 0.6s 0.2s ease both;opacity:0;">
        <div class="screen-header">
            ...
            <span style="margin-left:0.5rem;">AAPL · LIVE CHART PREVIEW</span>
        </div>
        <div style="padding:1rem 1.2rem;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.6rem;">
                <div>
                    <div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#eef2f7;line-height:1;">AAPL</div>
                    <div style="font-family:'IBM Plex Mono',monospace;font-size:0.6rem;color:#3a4a5e;">Apple Inc. · NASDAQ</div>
                </div>
    ...
    <div style="background:#0c1018;border:1px solid #1a2235;border-radius:8px;overflow:hidden;animation:fadeUp 0.6s 0.4s ease both;opacity:0;">
        ...
        {"".join(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:0.45rem 1rem;border-top:1px solid #1a2235;"><span style="font-family:IBM Plex Mono,monospace;font-size:0.7rem;color:#8896ab;">{m}</span><span style="font-family:IBM Plex Mono,monospace;font-size:0.65rem;color:{c};">{"●" if o else "○"} {s}</span></div>' for m,(s,c,o) in [("NYSE",(nyse_status,nyse_color,nyse_open)),("NASDAQ",(nas_status,nas_color,nas_open)),("CME Futures",(cme_status,cme_color,cme_open)),("Crypto",(cry_status,cry_color,cry_open))])}
    </div>
</div>
""", unsafe_allow_html=True)
```
...
features = [
(“📊”,“Backtest”,     “Test any strategy on years of real data. Returns, alpha, drawdown, Sharpe, win rate — all explained.”,   “pages/1_Backtest.py”),
...
(“▶”, “Replay”,       “Step through historical bars one at a time with 13+ overlays and 5 sub-charts. Buy/sell as you go.”,    “pages/3_Replay.py”),
(“🧠”,“Analysis”,     “Full fundamental + AI breakdown. Financials, valuation, risks, and a personalised verdict.”,             “pages/4_Analysis.py”),
(“📅”,“Earnings”,     “How did a stock react to every past earnings? Day-of move, pre-run, post follow-through — charted.”,    “pages/6_Earnings.py”),
(“💬”,“AI Coach”,     “Ask anything in plain English. Your coach explains results, strategies, and concepts at your level.”,    “pages/5_Assistant.py”),
]
fc = st.columns(6)
for col,(icon,title,desc,link) in zip(fc, features):
col.markdown(f’<div class="feat-card glow-border"><div class="feat-icon">{icon}</div><div class="feat-title">{title}</div><div class="feat-desc">{desc}</div></div>’, unsafe_allow_html=True)
col.page_link(link, label=“Open →”)

# ── Indicators showcase ────────────────────────────────────────────────────────

st.markdown(’<div class="divider" style="margin-top:2.5rem;">15 Indicators — Available Now</div>’, unsafe_allow_html=True)
...
tags = “”.join(f’<span style="display:inline-block;font-family:IBM Plex Mono,monospace;font-size:0.65rem;padding:3px 10px;border-radius:4px;background:{color}12;color:{color};border:1px solid {color}25;margin:3px 3px 3px 0;">{ind}</span>’ for ind in inds)
col.markdown(f”””
<div style="background:#0c1018;border:1px solid #1a2235;border-radius:{10}px;padding:1.3rem;height:100%;">
<div style="font-family:'IBM Plex Mono',monospace;font-size:0.6rem;color:{color};text-transform:uppercase;letter-spacing:0.18em;margin-bottom:0.9rem;">{cat}</div>
<div>{tags}</div>
...
hw = st.columns(5)
for col,(num,title,desc) in zip(hw,[
...
sl.markdown(f’<div class="row-item"><span class="tag" style="background:{tc}12;color:{tc};border:1px solid {tc}25;flex-shrink:0;">{tag}</span><div><div style="font-size:0.82rem;color:#eef2f7;">{name}</div><div style="font-size:0.72rem;color:#3a4a5e;margin-top:0.1rem;">{desc}</div></div></div>’, unsafe_allow_html=True)
...
(“Alpha”,             “Return above buy & hold. Positive = you actually added value.”),
...
sr.markdown(f’<div class="row-item"><div><div style="font-family:IBM Plex Mono,monospace;font-size:0.72rem;color:#00e676;margin-bottom:0.2rem;">{term}</div><div style="font-size:0.79rem;color:#3a4a5e;line-height:1.6;">{defn}</div></div></div>’, unsafe_allow_html=True)
